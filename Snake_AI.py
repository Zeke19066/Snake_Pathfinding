...
class SnekAI:
    ...
    def astar_local(self, maze, start, end, allow_diagonal_movement = False):
        ...
        f_mode = self.f_mode#localise this for speed.
        ...
        max_iterations = 150
        if f_mode == 2:
            max_iterations = int(max_iterations*0.6) #Since Sqrt is slow.
        ...
        while len(open_list) > 0:
            ...
            for new_position in adjacent_squares: # Adjacent squares
                ...
                if maze[node_y][node_x]: #using int(1) as bool
                    continue
                ...
                if len([1 for closed_child in closed_list if closed_child["position"] == child_pos]) > 0:
                    continue
                ...
                elif f_mode == 2: #cube then sqrt
                    child["h"] = isqrt((child_pos[0] - end[0]) ** 2) + ((child_pos[1] - end[1]) ** 2)
                elif f_mode == 3: #Manhattan heuristic
                    child["h"] = 1.1*(abs(child_pos[0] - end[0]) + abs(child_pos[1] - end[1]))
                
                child["f"] = child["g"] + child["h"]

                # Child is already in the open list
                if len([1 for open_node in open_list if open_node["position"] == child_pos and child["g"] > open_node["g"]]) > 0:
                    continue
